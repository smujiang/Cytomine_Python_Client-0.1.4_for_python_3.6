...
sys.path.append("/Users/general/Projects/GitHub/DigitalPathology/Cytomine\ Python\ Client-0.1.4/cytomine/lib")
...
class Cytomine(object):
    ...
    def __authorize(self, action, url="", content_type="", accept="application/json,*/*", sign_with_base_path = True):
        ...
        signature = base64.b64encode(hmac.new(self.__private_key.encode('utf-8'), message_to_sign.encode('utf-8'), sha1).digest())# modified by Jiang.Jun
        authorization = "CYTOMINE %s:%s" % (self.__public_key, signature.decode('utf-8'))
...
#        httplib2.debuglevel=4                
    ...
    def get_annotations(self, id_project = None, id_user = None, id_image = None, id_term = None, showGIS = None, showWKT = None, showMeta = None, bbox = None, id_bbox= None, reviewed_only = False):
        ...
        if bbox:
            query += "&bbox=%s" % urllib3.quote_plus(bbox)  # %str(bbox).strip('[]').replace(' ','%20')
        if id_bbox:
